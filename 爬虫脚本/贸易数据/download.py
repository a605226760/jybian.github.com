...
def res_csv(url):
    ...
    while True:
        try:
            ...
            print(proxy)
            if os.path.exists("./info/{0}+ps{1}+r{2}+rg{3}.csv".format(num,int(ps),int(r),rg)):
                ...
                if response.status_code == 200:
                    ...
                    with open("./info/{0}+ps{1}+r{2}+rg{3}.csv".format(num,int(ps),int(r),rg), 'wb') as f:
                        f.write(response.content)
                    print('文件提取完成')
                    size = os.path.getsize("./info/{0}+ps{1}+r{2}+rg{3}.csv".format(num,int(ps),int(r),rg))
                    if size >20000 or size ==611:
                        ...
                        global errow
                        errow += 1
                        os.remove("./info/{0}+ps{1}+r{2}+rg1.csv".format(num,int(ps),int(r)))
                        print('移除当前文件,重新爬取')
                        if errow > 3:
                            errow = 0
                            break
...
for rg in range(1,3):
    ...
    for r in tot_r:
        #改变年份
        for ps in tot_ps:
            url = 'http://comtrade.un.org/api/get?max=50000&type=C&freq=A&px=HS&ps={0}&r={1}&p=all&rg={2}&cc=AG2&fmt=csv'.format(int(ps),int(r),rg)
            print(url)
            IP_time += 1
